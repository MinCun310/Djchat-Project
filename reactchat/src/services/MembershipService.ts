...
const useMembership = (): IuseServer => {
    ...
    const joinServer = async (serverId: number): Promise<void> => {
        setIsLoading(true);
        try {
            await jwtAxios.post(`${BASE_URL}/djchat/membership/${serverId}/membership/`, {}, {withCredentials: true})
            setIsLoading(false)
    ...
    const leaveServer = async (serverId: number): Promise<void> => {
        setIsLoading(true);
        try {
            await jwtAxios.delete(`${BASE_URL}/djchat/membership/${serverId}/membership/`, {withCredentials: true});
            setIsLoading(false);
    ...
    const isMember = async (serverId: number): Promise<any> => {
        setIsLoading(true);
        try {
            const response = await jwtAxios.get(`${BASE_URL}/djchat/membership/${serverId}/membership/`, {withCredentials: true});
            setIsLoading(false);
